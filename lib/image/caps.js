 ...
 *
 *   'open'   → THE DEPLOYMENT ENFORCES NO SUBSCRIPTION AT ALL
 *              (helpers/subscriptionMode.js, Joel's call of 2026-09-04). This
 *              is the one status that is not a fact about the customer; it is
 *              a fact about the deployment, and checkSub is the only thing
 *              that sets it, so this file needs no knowledge of the switch.
 *
 *              It maps to the TOP tier rather than to "no limit", and that is
 *              a deliberate distinction: removing a PRICE is not the same as
 *              removing a SPEND LIMIT. Every generation is billed by Alibaba
 *              and stored as ~1.9 MB of BYTEA with no retention policy in this
 *              repo, so an unlimited cap is an unbounded invoice and an
 *              unbounded database — neither of which anybody asked for. 60 a
 *              day and 600 a month is not a paywall in any practical sense; it
 *              is the ceiling that stops one loop, one script or one bad
 *              afternoon from costing real money. Raise it by moving this to a
 *              more generous tier, in this one line.
...
function tierForSubscription(subscription) {
  const status = subscription && typeof subscription.status === 'string'
    ? subscription.status
    : null;
  if (status === 'open') return { tier: TIERS.agency, recognised: true };
