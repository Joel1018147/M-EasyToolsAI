...
const sizesModule = require('../sizes');
const { assertLegalSize, LEGAL_SIZES, DEFAULT_SIZE } = sizesModule;
...
function create(opts = {}) {
  ...
  return {
    ...
    /**
     * This provider's own size vocabulary, delegated to from ../index.js.
     * ADDED 2026-09-18 alongside nanobanana.js, whose aspect-ratio sizes are
     * not the same vocabulary as this file's `width*height` pixels — see
     * ../provider.js's interface doc. Pure passthrough: ../sizes.js already
     * WAS this provider's size module, this just exposes it through the
     * interface so ../index.js no longer has to import ../sizes.js directly.
     */
    resolveSize(requested) {
      return sizesModule.resolveSize(requested);
    },

    sizeCatalogue() {
      return sizesModule.catalogue();
    },
