/* ═══════════════════════════════════════════════════════════════════════════
   ...
   A provider module exports `create(opts) -> client`, and the client is:
     ...
     defaultSize()  string
     resolveSize(x) {ok:true,size}|{ok:false,code,message,legal} — the
                    provider's own validator. ../index.js delegates size
                    resolution HERE rather than to a shared module, because
                    DashScope's `1328*1328` and nanobanana's `16:9` are not
                    interchangeable vocabularies and a caller's stored preset
                    for one is not a legal value for the other. ADDED
                    2026-09-18 when a second provider with a genuinely
                    different size shape arrived; before that, ../index.js
                    called a shared ../sizes.js directly and this method
                    existed on the interface but nothing used it.
     sizeCatalogue() the full catalogue ../index.js's options() publishes,
                    analogous to legalSizes() but with the human-facing
...
const nanobanana = require('./providers/nanobanana');
...
const PROVIDERS = Object.freeze({
  ...
  [nanobanana.PROVIDER_NAME]: nanobanana,
});

/**
 * The provider used when a caller names none.
 *
 * CHANGED 2026-09-18 (Joel): DashScope/Qwen-Image -> nanobanana (Google Nano
 * Banana 2 Lite via fal.ai) — "qwen generates ai slop". DashScope stays
 * registered, not deleted: it is a working, tested integration and a
 * same-day rollback (change this one constant, or pass
 * `provider: 'dashscope'`) is cheaper insurance than re-writing it from
 * scratch if the new vendor underperforms. See
 * docs/BUILD_BRIEF_NANOBANANA_SWAP.md.
 */
const DEFAULT_PROVIDER = nanobanana.PROVIDER_NAME;
