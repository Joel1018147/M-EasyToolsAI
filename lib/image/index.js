  ...
  return {
    ...
    async generate({ user, subscription, input }) {
      ...
      const baseFields = {
        ...
        // The prompt stored here is the COMPOSED prompt (style + brand),
        // before any provider-specific wire transformation. For DashScope this
        // IS byte-identical to what was sent. For nanobanana it is NOT: that
        // provider has no native negative_prompt field, so it appends
        // "\n\nAvoid: <negativePrompt>" to the text it actually POSTs
        // (lib/image/providers/nanobanana.js, withAvoidClause) — the exact
        // wire prompt for that row is this column + negativePrompt below,
        // reconstructed, not stored verbatim a second time. See
        // test/image-contract.js §7(g), which asserts this deliberately.
